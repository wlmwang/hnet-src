...
int wTask::Output() {
    if (mSCType == 0 && mServer) {
        if (mServer->AddTask(this, EPOLLIN | EPOLLOUT, EPOLL_CTL_MOD, false) == -1) {
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::Output AddTask() failed", "");
            return -1;
        }
        ...
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::Output () failed", "mServer or mClient is null");
        return -1;
    }
    return 0;
}
    ...
    while (mRecvLen > sizeof(uint32_t)) {
        ...
        if (len > 0) {
            ...
            if (reallen < kMinPackageSize || reallen > kMaxPackageSize) {
                ret = -1;
                HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::TaskRecv () failed", ">0 message length error");
                break;

            } else if (reallen > static_cast<uint32_t>(len - sizeof(uint32_t))) {
                ret = 0;
                HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::TaskRecv () failed", ">0 recv a part of message");
                break;
            }
...
int wTask::Send2Buf(char cmd[], size_t len) {
    ...
    if (len < kMinPackageSize || len > kMaxPackageSize) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::Send2Buf () failed", "message too large");
        return -1;

    } else if (len > static_cast<size_t>(kPackageSize - mSendLen - sizeof(uint32_t))) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::Send2Buf () failed", "left buffer not enough");
        return -1;
    }
...
int wTask::SyncWorker(char cmd[], size_t len) {
    if (mServer && mServer->Worker()) {
        ...
        if (mServer->SyncWorker(cmd, len, kMaxProcess, &blackslot) == -1) {
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncWorker SyncWorker() failed", "");
            return -1;
        }
    } else {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncWorker () failed", "server is null");
        return -1;
    }
    return 0;
}
...
int wTask::AsyncWorker(char cmd[], size_t len) {
    if (mServer && mServer->Worker()) {
        ...
        if (mServer->AsyncWorker(cmd, len, kMaxProcess, &blackslot) == -1) {
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::AsyncWorker AsyncWorker() failed", "");
            return -1;
        }
    } else {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::AsyncWorker () failed", "server is null");
        return -1;
    }
    return 0;
}
...
int wTask::AsyncWorker(const google::protobuf::Message* msg) {
    if (mServer && mServer->Worker()) {
        ...
        if (mServer->AsyncWorker(msg, kMaxProcess, &blackslot) == -1) {
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::AsyncWorker SyncWorker() failed", "");
            return -1;
        }
    } else {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::AsyncWorker () failed", "server is null");
...
int wTask::AsyncSend(char cmd[], size_t len) {
	if (mSCType == 0 && mServer != NULL) {
        if (mServer->Send(this, cmd, len) == -1) {
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::AsyncSend Send() failed", "");
            return -1;
        }
        ...
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::AsyncSend () failed", "server or client is null");
        return -1;
	}
	return 0;
}
...
int wTask::SyncSend(char cmd[], size_t len, ssize_t *size) {
	...
	len += sizeof(uint8_t);
    if (len < kMinPackageSize || len > kMaxPackageSize) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncSend () failed", "message length error");
        return -1;
    }
...
int wTask::SyncRecv(char cmd[], ssize_t *size, size_t msglen, uint32_t timeout) {
    ...
    while (true) {
        ...
        if (timeout > 0) {
            ...
            if (tmleft > 0) {
                ...
                HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncRecv () failed", "timeout");
                return -1;
            }
        }
    ...
    if (len < kMinPackageSize || len > kMaxPackageSize) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncRecv () failed", "message length error,out range");
        return -1;

    } else if (len + sizeof(uint32_t) != recvheadlen + recvbodylen) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncRecv () failed", "message length error,illegal message");
        return -1;
    }

    *size = len - sizeof(uint8_t);
    if (msglen > 0 && msglen != static_cast<size_t>(*size)) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wTask::SyncRecv () failed", "message length error,error message");
        return -1;
    }
	...
	if (sp == kMpCommand) {
		...
		if (basecmd->GetId() == CmdId(kCmdNull, kParaNull)) {
			...
			if (mEventCmd(basecmd->GetId(), &request) == false) {
                ...
                HNET_ERROR(soft::GetLogPath(), "%s : %s[id=%s]", "wTask::Handlemsg () failed", "request invalid", id.c_str());
                ret = -1;
			}
		}
		...
		if (mEventPb(name, &request) == false) {
            HNET_ERROR(soft::GetLogPath(), "%s : %s[name=%s]", "wTask::Handlemsg () failed", "request invalid", name.c_str());
            ret = -1;
		}
#else
        HNET_ERROR(soft::GetLogPath(), "%s : %s[sp=%d]", "wTask::Handlemsg () failed", "protobuf invalid", sp);
        ret = -1;
#endif
	} else {
        HNET_ERROR(soft::GetLogPath(), "%s : %s[sp=%d]", "wTask::Handlemsg () failed", "request invalid", sp);
        ret = -1;
	}
