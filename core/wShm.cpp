...
int wPosixShm::CreateShm(int pipeid) {
	...
	if (fd == -1) {
		HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::CreateShm open() failed", error::Strerror(errno).c_str());
		return -1;
	}
	close(fd);
	...
	if (key == -1) {
		HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::CreateShm ftok() failed", error::Strerror(errno).c_str());
		return -1;
	}
	...
	if (mShmId == -1) {
		// 申请内存失败
		if (errno != EEXIST) {
			HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::CreateShm shmget() failed", error::Strerror(errno).c_str());
			return -1;
		}
		...
		if (mShmId == -1) {
			...
			if (mShmId == -1) {
				...
				if (shmctl(mShmId, IPC_RMID, NULL) == -1) {
					HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::CreateShm shmctl() failed", error::Strerror(errno).c_str());
					return -1;
				}
	...
	if ((intptr_t)addr == -1) {
		HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::CreateShm shmat() failed", error::Strerror(errno).c_str());
		return -1;
    }
...
int wPosixShm::AttachShm(int pipeid) {
	key_t key = ftok(mFilename.c_str(), pipeid);
	if (key == -1) {
		HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::AttachShm ftok() failed", error::Strerror(errno).c_str());
		return -1;
	}

	mShmId = shmget(key, mSize, 0666);
	if (mShmId == -1) {
		HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::AttachShm shmget() failed", error::Strerror(errno).c_str());
		return -1;
	}
	...
	void* addr = shmat(mShmId, NULL, 0);
	if ((intptr_t)addr == -1) {
		HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::AttachShm shmat() failed", error::Strerror(errno).c_str());
		return -1;
    }
    // 存储头信息
	mShmhead = reinterpret_cast<struct Shmhead_t*>(addr);
...
void* wPosixShm::AllocShm(size_t size) {
	...
	HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::AllocShm failed, shm space not enough", "");
	return NULL;
}
...
void wPosixShm::Destroy() {
    ...
    if (mShmId > 0 && shmctl(mShmId, IPC_RMID, NULL) == -1) {
    	HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::Destroy shmctl() failed", error::Strerror(errno).c_str());
    }
}
...
void wPosixShm::Remove() {
    ...
    if (mShmhead && shmdt(reinterpret_cast<void*>(mShmhead->mStart)) == -1) {
    	HNET_ERROR(soft::GetLogPath(), "%s : %s", "wPosixShm::Destroy shmdt() failed", error::Strerror(errno).c_str());
    }
}
