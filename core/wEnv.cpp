...
public:
	PosixSequentialFile(const std::string& fname, FILE* f) : mFilename(fname), mFile(f) { }
	virtual ~PosixSequentialFile() { fclose(mFile); }
...
public:
	PosixRandomAccessFile(const std::string& fname, int fd) : mFilename(fname), mFD(fd) { }
	virtual ~PosixRandomAccessFile() { close(mFD); }
