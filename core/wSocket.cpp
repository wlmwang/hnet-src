    ...
    while (true) {
        *size = recv(mFD, reinterpret_cast<void*>(buf), len, 0);
        ...
        if (*size > 0) {
            ...
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wSocket::RecvBytes recv() failed", error::Strerror(errno).c_str());
            ret = -1;
            break;
        }
    }
    ...
    while (leftlen > 0) {
        *size = send(mFD, reinterpret_cast<void*>(buf + sendedlen), leftlen, 0);
        ...
        if (*size >= 0) {
            ...
            HNET_ERROR(soft::GetLogPath(), "%s : %s", "wSocket::SendBytes send() failed", error::Strerror(errno).c_str());
            ret = -1;
            break;
        }
    }
...
int wSocket::Close() {
    int ret = close(mFD);
    if (ret == -1) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wSocket::Close close() failed", error::Strerror(errno).c_str());
    }
    mFD = kFDUnknown;
    return ret;
}
...
int wSocket::GetNonblock() {
    ...
    if (ret == -1) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wSocket::GetNonblock fcntl() failed", error::Strerror(errno).c_str());
        return -1;
    }
...
int wSocket::SetNonblock(bool nonblock) {
    ...
    if (ret == -1) {
        HNET_ERROR(soft::GetLogPath(), "%s : %s", "wSocket::SetNonblock fcntl() O_NONBLOCK failed", error::Strerror(errno).c_str());
    }
    return ret;
}
