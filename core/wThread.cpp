...
int wThread::StartThread() {
	...
	if (pthread_attr_init(&mAttr) != 0) {
		LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread pthread_attr_init() failed", error::Strerror(errno).c_str());
		return -1;
	}
	...
	if (pthread_attr_setscope(&mAttr, PTHREAD_SCOPE_SYSTEM) != 0) {
		LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread pthread_attr_setscope() failed", error::Strerror(errno).c_str());
		return -1;
	}

	if (mJoinable) {	// 结合线程
		if (pthread_attr_setdetachstate(&mAttr, PTHREAD_CREATE_JOINABLE) != 0) {
			LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread pthread_attr_setdetachstate(PTHREAD_CREATE_JOINABLE) failed", error::Strerror(errno).c_str());
			return -1;
		}
	} else {	// 分离线程
		if (pthread_attr_setdetachstate(&mAttr, PTHREAD_CREATE_DETACHED) == -1) {			
			LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread pthread_attr_setdetachstate(PTHREAD_CREATE_DETACHED) failed", error::Strerror(errno).c_str());
			return -1;
		}
	}

	if (PrepareThread() == -1) {
		LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread PrepareThread() failed", "");
		return -1;
	}

	// 创建线程
	if (pthread_create(&mPthreadId, &mAttr, &wThread::ThreadWrapper, reinterpret_cast<void*>(this)) != 0) {
		LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread pthread_create() failed", error::Strerror(errno).c_str());
		return -1;
	}
	...
	if (pthread_attr_destroy(&mAttr) != 0) {
		LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::StartThread pthread_attr_destroy() failed", error::Strerror(errno).c_str());
		return -1;
	}
    return 0;
}
...
int wThread::JoinThread() {
	if (0 != mPthreadId && mJoinable) {
	    ...
	    if (pthread_join(mPthreadId, NULL) != 0) {
	    	if (errno != ESRCH) {
				LOG_ERROR(soft::GetLogPath(), "%s : %s", "wThread::JoinThread pthread_join() failed", error::Strerror(errno).c_str());
				return -1;
	    	}
	    }
	    mPthreadId = 0;
